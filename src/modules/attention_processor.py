...
class MaskedIPAttnProcessor2_0(nn.Module):
    ...
    def prepare_attention_mask_qk(self, boxes, phrase_idxes, sequence_length_q, sequence_length_k,
        ...
        attention_mask_qk_image = (1 - attention_mask_qk_image.to(dtype)) * -10000.0
            ...
            for j in range(num_ref):
                ...
                if start_idx == 0 and end_idx == 0:
                    ...
                    attention_map = attention_probs[i, :, :,
                                    start_idx:end_idx]
                    attention_map = torch.mean(torch.mean(attention_map, dim=-1), dim=0)
    ...
    def get_subject_attention_maps(self, attention_probs, boxes, subject_phrase_idxes, head_size):
        """Extract the text cross-attention maps corresponding to each subject phrase for the attention-bbox alignment auxiliary loss.
        Store them in attn_store with the key set to place_in_unet+"_subjects", and the shape is (bsz, num_subjects, h, w).
        Only layers with a resolution ≤32×32 (semantic layout layers) are stored, consistent with the filtering condition of get_predicate_attention_maps.
        """
        if subject_phrase_idxes is None or boxes is None:
            return
        ...
        for i in range(bsz):
            sample_attention_maps = []
            for j in range(num_ref):
                start_idx, end_idx = int(subject_phrase_idxes[i, j, 0].item()), int(subject_phrase_idxes[i, j, 1].item())
                if start_idx == 0 and end_idx == 0:

                    sample_attention_maps.append(
                        torch.zeros(num_tokens_q, dtype=attention_probs.dtype, device=attention_probs.device))
                else:
                    attention_map = attention_probs[i, :, :, start_idx:end_idx]
                    attention_map = torch.mean(torch.mean(attention_map, dim=-1), dim=0)
                    sample_attention_maps.append(attention_map)
            batch_attention_maps.append(torch.stack(sample_attention_maps))

        if num_tokens_q <= 32 ** 2 and self.attn_store is not None:
            self.attn_store[self.place_in_unet + "_subjects"] = torch.stack(batch_attention_maps).reshape(bsz, num_ref, h, w)
    ...
    def get_psuedo_attention_mask(self, head_size):
        ...
        if not self.use_psuedo_attention_mask or len(self.attention_maps) < self.start_step:
            return None, None
        text_attention_maps = torch.stack(self.attention_maps).mean(dim=0)
        ...
        text_attention_maps = text_attention_maps.transpose(1, 2)
        ...
        psuedo_attention_mask = (1 - psuedo_attention_mask) * -10000.0
    ...
    def __call__(
        ...
        if not self.need_text_attention_map:
            ...
            self.get_subject_attention_maps(attention_probs, boxes, subject_phrase_idxes, attn.heads)
